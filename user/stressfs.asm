...
main(int argc, char *argv[])
{
  ...
  8e:	340080e7          	jalr	832(ra) # 3ca <open>
  92:	892a                	mv	s2,a0
  94:	44d1                	li	s1,20
  for(i = 0; i < 20; i++)
//    printf(fd, "%d\n", i);
  ...
  a4:	30a080e7          	jalr	778(ra) # 3aa <write>
  for(i = 0; i < 20; i++)
  ...
  b2:	304080e7          	jalr	772(ra) # 3b2 <close>
  ...
  d0:	2fe080e7          	jalr	766(ra) # 3ca <open>
  d4:	892a                	mv	s2,a0
  d6:	44d1                	li	s1,20
  for (i = 0; i < 20; i++)
  ...
  e6:	2c0080e7          	jalr	704(ra) # 3a2 <read>
  for (i = 0; i < 20; i++)
...
gets(char *buf, int max)
{
 ...
 1fe:	1a8080e7          	jalr	424(ra) # 3a2 <read>
    if(cc < 1)
...
stat(const char *n, struct stat *st)
{
 ...
 250:	17e080e7          	jalr	382(ra) # 3ca <open>
  if(fd < 0)
 ...
 260:	186080e7          	jalr	390(ra) # 3e2 <fstat>
 264:	892a                	mv	s2,a0
  close(fd);
 266:	8526                	mv	a0,s1
 268:	00000097          	auipc	ra,0x0
 26c:	14a080e7          	jalr	330(ra) # 3b2 <close>
  return r;
}
 270:	854a                	mv	a0,s2
